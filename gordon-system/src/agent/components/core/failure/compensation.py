...
class CompensationAction:
    ...
    retry_policy: "RetryPolicy" = field(
        default_factory=lambda: RetryPolicy(max_attempts=3, backoff_seconds=1.0)
    )
...
class RetryPolicy:
    """
    Policy for retrying compensation actions.
    
    Args:
        max_attempts: Maximum number of retry attempts
        backoff_seconds: Base seconds between retries
